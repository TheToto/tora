...
class ToraProtocol {
    ...
    connect() {
        if (this.useWebSocket) {
            this.sock = new isomorphic_ws_1.WebSocket(this.webSocketBridge || `${this.port === 443 ? "wss" : "ws"}://${this.host}:${this.port}`);
    ...
    close() {
        try {
            if (this.sock instanceof isomorphic_ws_1.WebSocket) {
                this.sock.close();
            }
    ...
    onSocketData(data) {
        ...
        if (data) {
            ...
            if (data instanceof buffer_1.Buffer)
                ...
                bytes = new buffer_1.Buffer(data);
            else if (typeof data === "string")
                bytes = buffer_1.Buffer.from(data); // This should never append.
            else
